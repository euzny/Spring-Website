...
public class PageVO {
	...
	int totalList; 			// 총 글의 개수 (DB에서 조회해온 전체 글 수)
	...
	//전체 페이지가 조회가 되면 페이지 처리를 할 수 있기 때문에
	// 조회된 전체 페이지 수 값이 등록되는 단계에서
	// 페이지 처리에 관한 작업을 하면 되기에 totalList setter 메소드에서 처리를 함
	public void setTotalList(int totalList) {
		this.totalList = totalList; //DB에서 총 글의 건수
		//총 페이지 수(totalPage) : 8 페이지 = totalList / pageList = 7 ... 2
		totalPage = totalList / pageList;
		// 총 384건의 글을 페이지당 10건씩 총 페이지수 38 
		// 나머지가 발생하기 때문에 1페이지를 증가해야함
		...
		// 마지막 블럭에서 끝 페이지번호가 총 페이지수보다 클 수 없으므로 총 페이지수를 끝 페이지번호로 한다. 
